...
app = Flask(__name__)
app.secret_key = "eyin_alerts_v39"
...
def get_layout(content, settings):
    # نظام عرض رسائل التنبيه (Flash)
    messages = ""
    with app.app_context():
        from flask import get_flashed_messages
        flashes = get_flashed_messages(with_categories=true)
        for category, message in flashes:
            color = "#ef4444" if category == 'error' else "#22c55e"
            messages += f'<div style="background:{color}; color:white; padding:10px; border-radius:10px; margin-bottom:15px; text-align:center;">{message}</div>'
    ...
    <head>
        ...
        <style>
            :root {{ --bg: #0f172a; --card: #1e293b; --text: #f8fafc; --border: #334155; --primary: #818cf8; }}
            [data-theme="light"] {{ --bg: #ffffff; --card: #f8fafc; --text: #1e293b; --border: #e2e8f0; --primary: #4f46e5; }}
    ...
    <body data-theme="dark">
        ...
        <div style="max-width:400px; margin: 20px auto;">{messages}</div>
        {content}
    </body>
    </html>
    '''
...
def login():
    if request.method == 'POST':
        ...
        if u:
            session['user_id']=u['id']; session['user_phone']=u['phone']
            return redirect('/')
        else:
            flash('الرقم أو كلمة المرور غير صحيحة', 'error')
    ...
    if request.method == 'POST':
        if request.form.get('otp') == otp:
            try:
                ...
                flash('عذراً، هذا الرقم مسجل مسبقاً في متجر عين', 'error')
                return redirect('/register')
    return get_layout(f'''<div class="auth-box"><div class="auth-card"><h2>التحقق</h2><a href="https://wa.me/{ADMIN_WHATSAPP}?text=كود التفعيل:{otp}" target="_blank" style="background:#25d366; color:white; padding:15px; text-decoration:none; border-radius:10px; display:block; margin-bottom:10px;">اطلب الكود عبر واتساب</a><form method="post"><input type="number" name="otp" required><button type="submit" class="main-btn">تأكيد</button></form></div></div>''', get_safe_settings())

# مسارات الإدارة و "إعلاناتي" (نفس v38)
...
def admin():
    ...
    return f'<div dir="rtl" style="padding:20px; background:white; color:black;"><h2>الإدارة</h2><form method="post" enctype="multipart/form-data"><input type="hidden" name="action" value="update_logo"><input type="file" name="logo_file"><button type="submit">تحديث</button></form><hr><form method="post"><input type="hidden" name="action" value="update_name"><input type="text" name="store_name" value="{st["store_name"]}"><button type="submit">حفظ</button></form><hr>{p_list}</div>'
