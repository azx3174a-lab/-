...
<head>
    ...
    <style>
        ...
        .auth-box { max-width: 350px; margin: 80px auto; text-align: center; background: var(--card); padding: 30px; border-radius: 25px; border: 1px solid var(--border); }
...
@app.route('/')
def index():
    # إذا لم يسجل دخول، تظهر واجهة إنشاء الحساب فوراً
...
def register():
    ...
    return wrap('''
    <div class="auth-box">
        <h2 style="margin-top:0;">إنشاء حساب جديد بالرقم</h2>
        <form method="post">
            ...
            <input type="password" name="password" placeholder="اختر كلمة مرور" required>
            <button type="submit" class="main-btn">تسجيل</button>
        </form>
        <p style="font-size:14px; margin-top:20px;">لديك حساب؟ <a href="/login" style="color:var(--primary); text-decoration:none; font-weight:bold;">تسجيل دخول</a></p>
    </div>
    ''')
...
def login():
    ...
    <div class="auth-box">
        ...
        <p style="font-size:14px; margin-top:20px;">ليس لديك حساب؟ <a href="/register" style="color:var(--primary); text-decoration:none; font-weight:bold;">أنشئ حسابك</a></p>
    </div>
    ''')
...
def verify():
    ...
    wa = f"https://wa.me/{ADMIN_WHATSAPP}?text=كود تفعيل متجر عين لرقمي هو: {otp}"
    if request.method == 'POST':
    ...
    <div class="auth-box">
        <h2>تحقق من رقمك</h2>
        <p style="font-size:14px; opacity:0.8;">اضغط الزر بالأسفل لطلب كود التفعيل عبر الواتساب</p>
        <a href="{wa}" target="_blank" style="background:#25d366; color:white; padding:15px; text-decoration:none; border-radius:12px; display:block; font-weight:bold; margin-bottom:20px;">💬 اطلب الكود واتساب</a>
        <form method="post">
            <input type="number" name="otp" placeholder="أدخل الكود هنا" required style="text-align:center; font-size:20px;">
            <button type="submit" class="main-btn">تأكيد الحساب</button>
        </form>
    </div>
    ''')
...
def my_ads():
    ...
    cards = "".join([f'<div class="card"><img src="{p["image_url"]}" style="width:100%; height:150px; object-fit:contain;"><h3>{p["name"]}</h3><form action="/delete_product" method="post" onsubmit="return confirm(\'حذف نهائي؟\')"><input type="hidden" name="id" value="{p["id"]}"><button type="submit" style="background:#ef4444; color:white; border:none; width:100%; padding:8px; border-radius:8px; cursor:pointer;">حذف</button></form></div>' for p in prods])
    return wrap(f'<div style="padding:20px; max-width:1000px; margin:auto;"><h2>📦 إعلاناتي</h2><a href="/" style="text-decoration:none; color:var(--primary);">⬅️ عودة</a><div class="grid">{cards}</div></div>')
...
def admin():
    ...
    conn = get_db_connection(); cur = conn.cursor(cursor_factory=DictCursor)
    cur.execute("SELECT * FROM products ORDER BY id DESC"); prods = cur.fetchall(); cur.close(); conn.close()
    p_list = "".join([f'<div style="border-bottom:1px solid #ddd; padding:10px;">{p["name"]} <form method="post" style="display:inline;"><input type="hidden" name="action" value="delete"><input type="hidden" name="id" value="{p["id"]}"><button type="submit" style="color:red; background:none; border:none; cursor:pointer;">[حذف]</button></form></div>' for p in prods])
    return f'<div dir="rtl" style="padding:20px; font-family:sans-serif;"><h2>🛠️ لوحة الإدارة</h2>{p_list}<br><a href="/">عودة</a></div>'
