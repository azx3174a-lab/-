...
app = Flask(__name__)
app.secret_key = "eyin_master_v62_final"
...
def init_db():
    try:
        ...
        cur.execute("CREATE TABLE IF NOT EXISTS products (id SERIAL PRIMARY KEY, name TEXT, price TEXT, image_url TEXT, whatsapp TEXT, user_id INTEGER)")
...
<head>
    ...
    <style>
        ...
        .auth-link { display: inline-block; margin-top: 15px; color: var(--primary); text-decoration: none; font-size: 14px; font-weight: bold; }
        ...
        .btn-del { color: #ef4444; background: none; border: none; cursor: pointer; font-weight: bold; padding: 5px; }
        .btn-ban { color: var(--primary); background: none; border: none; cursor: pointer; font-weight: bold; padding: 5px; }
    </style>
</head>
...
def index():
    ...
    html = '<div class="grid">'
    for p in prods:
        html += f'''<div class="card"><img src="{p['image_url']}"><div class="card-info"><h3 class="prod-name">{p['name']}</h3><p style="margin:2px 0; font-size:11px; color:gray;">بواسطة: {p['username']}</p><div style="color:var(--primary); font-weight:bold;">{p['price']} ريال</div><a href="https://wa.me/{p['whatsapp']}" style="color:#22c55e; text-decoration:none; font-size:13px; font-weight:bold; margin-top:5px;">💬 تواصل</a></div></div>'''
    return render_template_string(LAYOUT.replace('{% block content %}{% endblock %}', html + '</div>' + '<div id="addModal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.8); align-items:center; justify-content:center; z-index:100;"><div style="background:var(--card); padding:20px; border-radius:20px; width:90%; max-width:400px;"><h3>نشر إعلان</h3><form action="/add_public" method="post" enctype="multipart/form-data"><input type="text" name="name" placeholder="المنتج" required><input type="number" name="price" placeholder="السعر" required><input type="file" name="image_file" required><button type="submit" class="main-btn">نشر</button><button type="button" onclick="document.getElementById(\'addModal\').style.display=\'none\'" style="width:100%; background:none; border:none; color:gray; cursor:pointer;">إلغاء</button></form></div></div>'), st=st)
...
def admin():
    ...
    for u in users:
        ...
        <form method="post" style="display:inline;"><input type="hidden" name="user_id" value="{u["id"]}"><input type="hidden" name="action" value="{"unban_user" if u["is_banned"] else "ban_user"}"><button type="submit" class="btn-ban">{"فك" if u["is_banned"] else "حظر"}</button></form>
        <form method="post" style="display:inline;"><input type="hidden" name="user_id" value="{u["id"]}"><input type="hidden" name="action" value="delete_user"><button type="submit" class="btn-del" onclick="return confirm('حذف نهائي؟')">مسح</button></form>
        </td></tr>'''
    content = f'''<div style="max-width:800px; margin:auto; padding:10px;"><h3>⚙️ الإعدادات</h3><form method="post" enctype="multipart/form-data"><input type="hidden" name="action" value="update_settings">اسم المتجر: <input type="text" name="store_name" value="{st["store_name"]}"><br>الشعار: <input type="file" name="logo_file"><button type="submit" class="main-btn">حفظ</button></form><hr style="border-color:var(--border); margin:20px 0;"><h3>👤 المستخدمين</h3><table class="admin-table"><thead><tr><th>الاسم</th><th>الجوال</th><th>📌</th><th>أمر</th></tr></thead><tbody>{u_html}</tbody></table><br><a href="/" style="color:var(--primary); text-decoration:none;">⬅️ العودة</a></div>'''
    return render_template_string(LAYOUT.replace('{% block content %}{% endblock %}', content), st=st)

# باقي المسارات (Login, Register, Profile, الخ) كما في v61
...
def register():
    ...
    return render_template_string(LAYOUT.replace('{% block content %}{% endblock %}', '<div style="max-width:350px; margin:40px auto; text-align:center;"><h2>إنشاء حساب</h2><form method="post"><input type="text" name="username" placeholder="الاسم" required><input type="number" name="phone" placeholder="الجوال 5" required><input type="password" name="password" placeholder="كلمة المرور" required><button type="submit" class="main-btn">سجل</button></form><p style="margin-top:20px; font-size:14px;">عندك حساب؟ <a href="/login" class="auth-link" style="margin-top:0;">سجل دخول</a></p></div>'), st=get_st())
...
def profile():
    ...
    for p in prods:
        html += f'<div class="card"><img src="{p["image_url"]}"><div class="card-info"><h3 class="prod-name">{p["name"]}</h3><form action="/delete_product" method="post"><input type="hidden" name="id" value="{p["id"]}"><button type="submit" style="color:red; background:none; border:none; cursor:pointer;">حذف</button></form></div></div>'
...
def add_public():
    ...
    conn = get_db_connection(); cur = conn.cursor(); cur.execute("INSERT INTO products (user_id, name, price, image_url, whatsapp) VALUES (%s, %s, %s, %s, %s)", (session['user_id'], request.form.get('name'), request.form.get('price'), img, session['user_phone']))
