...
app = Flask(__name__)
app.secret_key = "eyin_otp_system_v22"
...
def register():
    if request.method == 'POST':
        ...
        # تخزين البيانات مؤقتاً في الجلسة لحين التحقق
        session['temp_user'] = {'phone': full_phone, 'password': password, 'otp': otp_code}
        
        # تنبيه: في الواقع هنا يتم إرسال SMS. حالياً سنطبعه في الـ Logs
        print(f"--- [EYIN STORE] OTP CODE FOR {full_phone} IS: {otp_code} ---")
    ...
    return render_template_string('''
        <div dir="rtl" style="font-family:sans-serif; text-align:center; padding:50px;">
            ...
            <form method="post" style="display:inline-block; width:90%; max-width:350px;">
                <div style="display:flex; align-items:center; background:#eee; border-radius:10px; padding:5px; margin-bottom:10px;">
                    <span style="padding:10px; font-weight:bold; color:#555;">966+</span>
                ...
                <input type="password" name="password" placeholder="اختر كلمة مرور" required style="width:100%; padding:15px; border-radius:10px; border:1px solid #ddd; margin-bottom:15px; box-sizing:border-box;">
                <button type="submit" style="width:100%; padding:15px; background:#25d366; color:white; border:none; border-radius:10px; font-weight:bold; cursor:pointer;">إرسال كود التحقق</button>
            </form>
            <p style="margin-top:20px;">لديك حساب؟ <a href="/login" style="color:#4f46e5; text-decoration:none;">تسجيل دخول</a></p>
        </div>
    ''')
...
def verify_otp():
    ...
    if request.method == 'POST':
        ...
        if user_otp == temp['otp']:
            ...
            try:
                ...
                conn.commit()
                flash('✅ تم التحقق وإنشاء الحساب بنجاح!', 'success')
    ...
    return render_template_string('''
        <div dir="rtl" style="font-family:sans-serif; text-align:center; padding:50px;">
            <h2>تحقق من رقمك</h2>
            <p>أدخل الكود المكون من 4 أرقام المرسل إلى {{ session['temp_user']['phone'] }}</p>
            <form method="post" style="display:inline-block; width:300px;">
                <input type="number" name="otp" placeholder="0 0 0 0" required style="width:100%; padding:15px; text-align:center; font-size:24px; letter-spacing:10px; border-radius:10px; border:2px solid #4f46e5; margin-bottom:20px;">
                <button type="submit" style="width:100%; padding:15px; background:#4f46e5; color:white; border:none; border-radius:10px; font-weight:bold;">تأكيد الحساب</button>
            </form>
            <br><br><a href="/register" style="color:#muted; font-size:12px;">تغيير الرقم؟</a>
        </div>
    ''')

# بقية المسارات (Login, Index, Admin) تبقى كما هي مع تحديث 966 في تسجيل الدخول
    ...
    return render_template_string('''
        <div dir="rtl" style="font-family:sans-serif; text-align:center; padding:50px;">
            ...
            <form method="post" style="display:inline-block; width:300px;">
                ...
                <input type="password" name="password" placeholder="كلمة المرور" required style="width:100%; padding:12px; margin-bottom:10px; border-radius:10px; border:1px solid #ddd;">
                <button type="submit" style="width:100%; padding:12px; background:#4f46e5; color:white; border:none; border-radius:10px;">دخول</button>
            </form>
            <p>ليس لديك حساب؟ <a href="/register">إنشاء حساب</a></p>
        </div>
    ''')

# --- الصفحة الرئيسية --- (نفس الكود السابق مع تجميل بسيط)
@app.route('/')
def index():
    conn = get_db_connection(); cur = conn.cursor(cursor_factory=DictCursor)
    ...
    <head>
        ...
        <style>
            ...
            .btn-buy { background: #25d366; color: white; text-decoration: none; display: block; text-align: center; padding: 12px; border-radius: 12px; font-weight: bold; margin-top: 10px; }
        </style>
    </head>
    <body>
        <div class="header">
            ...
            <div style="display:flex; gap:10px; align-items:center;">
                {% if session.get('user_id') %}
                    <button onclick="document.getElementById('addModal').style.display='flex'" style="background:var(--primary); color:white; border:none; padding:8px 12px; border-radius:8px;">+ إعلان</button>
                    <a href="/logout_user" style="text-decoration:none; color:red; font-size:12px;">خروج</a>
                {% else %}
...
# (الدوال الأخرى مثل add_public و logout_user و admin تبقى كما هي من النسخة السابقة)
