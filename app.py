...
app = Flask(__name__)
app.secret_key = "eyin_secure_market_v15"
...
# تحديث الجدول لدعم الرقم السري للمنتج
def init_db():
...
def index():
    ...
    # التعامل مع محاولة الحذف بالرقم السري
    if request.method == 'POST' and request.form.get('action') == 'delete':
        ...
        if result and result['delete_code'] == user_code:
            ...
            # هنا ممكن نضيف تنبيه أن الكود خطأ (اختياري)
            pass
    ...
    <head>
        ...
        <title>متجر عين - منصة العرض الآمنة</title>
        <style>
            ...
            .header { display: flex; justify-content: space-between; align-items: center; max-width: 1100px; margin: auto; gap: 15px; }
            ...
            .card { background: var(--card); padding: 20px; border-radius: 24px; box-shadow: 0 10px 15px -3px rgba(0,0,0,0.05); border: 1px solid var(--border); }
            ...
            .delete-box { border-top: 1px solid var(--border); pt: 10px; display: flex; gap: 5px; }
            .delete-input { font-size: 12px; padding: 5px; margin: 0; border: 1px solid #ef4444; border-radius: 5px; width: 60px; }
    ...
    <body>
        ...
        <div id="addModal" class="modal">
            <div class="modal-content">
                <h3 style="margin-top:0;">إضافة إعلان جديد</h3>
                <form action="/add_public" method="post" enctype="multipart/form-data">
                    ...
                    <input type="text" name="whatsapp" placeholder="رقم واتسابك (مثال: 96650...)" required>
                    <input type="password" name="delete_code" placeholder="اختر كود سري لحذف إعلانك لاحقاً" required>
                    <label style="font-size: 12px; color: var(--muted);">صورة المنتج:</label>
                    <input type="file" name="image_file" required>
                    <button type="submit" class="add-btn" style="width:100%; margin-top: 10px;">نشر الإعلان</button>
...
def add_public():
    ...
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("INSERT INTO products (name, price, image_url, whatsapp, delete_code) VALUES (%s, %s, %s, %s, %s)",
                (name, price, image_data, whatsapp, delete_code))
    conn.commit()
    cur.close()
    conn.close()
...
def admin():
    if request.method == 'POST':
        if request.form.get('password') == '317400':
            ...
            conn = get_db_connection()
            cur = conn.cursor()
            if request.form.get('action') == 'update_logo':
    ...
    return render_template_string('<div dir="rtl" style="text-align:center;padding:50px;"><h2>تغيير الشعار</h2><form method="post" enctype="multipart/form-data"><input type="hidden" name="action" value="update_logo"><input type="file" name="logo_file" required><button type="submit">تحديث</button></form><br><a href="/">العودة</a></div>')
